...
#pragma warning disable 649
    ...
    internal struct KEYBDINPUT
    {
        ...
        /// </summary>
        public readonly UInt16 KeyCode;
        ...
        public KEYBDINPUT(UInt16 keyCode, UInt16 scan, UInt32 flags)
        {
            KeyCode = keyCode;
            Scan = scan;
            if (keyCode == 0 && (Scan & 0xFF00) == 0xE000)
            {
                // Handle extended keys:
                // If the scan code is preceded by a prefix byte that has the value 0xE0 (224),
                // we need to include the KEYEVENTF_EXTENDEDKEY flag in the Flags property.    
                flags |= (UInt32) KeyboardFlag.ExtendedKey;
            }
            
            if (scan == 0 && InputBuilder.IsExtendedKey((VirtualKeyCode) keyCode))
            {
                flags |= (UInt32) KeyboardFlag.ExtendedKey;
            }
            Flags = flags;
